...
----------------------------- Eq -------------------------
 ...
 let
  safe3ListsFromSameVectorsAreEq = TestCase
   (do
      env <- EnvLdr.loadTestEnvironment
      safeVertex <- HexR.runEitherIO "safeVertex" $ L3.toSafeList3 [Geo.newVertex  1 1 1, Geo.newVertex  2 2 2, Geo.newVertex  3 3 3]
 ...
 let
  safe3ListsFromDiffVectorsAreNotEq = TestCase
   (do
      env <- EnvLdr.loadTestEnvironment
      safeVertexs1 <- HexR.runEitherIO "safeVertex1" $ L3.toSafeList3 [Geo.newVertex  1 1 1, Geo.newVertex  2 2 2, Geo.newVertex  3 3 3]
      safeVertexs2 <- HexR.runEitherIO "safeVertex2" $ L3.toSafeList3 [Geo.newVertex  1 1 1, Geo.newVertex  2 2 2, Geo.newVertex  33 33 33]
...
------------ create points from [vertex] with length 2 - 5 ----------------------------------
 ...
 let
  create3PointsFrom3Vertex = TestCase
   (do
      env <- EnvLdr.loadTestEnvironment
      safeVertexs <- HexR.runEitherIO "safeVertex" $ L3.toSafeList3 [Geo.newVertex  1 2 3, Geo.newVertex  4 5 6, Geo.newVertex  7 8 9]
      points <- runRIO env $ Pnt.toPoints safeVertexs
      assertEqual "create points from safeList3 vertex length == 3"
       [1, 2, 3]
       (map Env.evalPointId (L3.evalSafeList3 points))
   )
 ...
 let
  create4PointsFrom4Vertex = TestCase
   (do
      env <- EnvLdr.loadTestEnvironment
      safeVertexs <- HexR.runEitherIO "safeVertex" $ L3.toSafeList3 [Geo.newVertex 1 1 1, Geo.newVertex 2 2 2, Geo.newVertex 3 3 3, Geo.newVertex 4 4 4]
      points <- runRIO env $ Pnt.toPoints safeVertexs
...
------------------------------------------------------------------------------------------------------------------------
 ...
 let
  unique3VertexToPoints = TestCase
   (do
      env <- EnvLdr.loadTestEnvironment
      safeVertexs  <- HexR.runEitherIO "fjjf" $ ((L3.toSafeList3 [Geo.newVertex  1 1 1, Geo.newVertex  2 2 2, Geo.newVertex  3 3 3]):: Either Hex.HasMeshException L3.VertexSafe3List)
      points <- runRIO env $ Pnt.toPoints safeVertexs
 ...
 let
  unique4VertexToPoints = TestCase
   (do
      env <- EnvLdr.loadTestEnvironment
      safeVertexs  <- HexR.runEitherIO "safe vertex" $ ((L3.toSafeList3 [Geo.newVertex  1 1 1, Geo.newVertex  2 2 2, Geo.newVertex  3 3 3, Geo.newVertex 4 4 4]):: Either Hex.HasMeshException L3.VertexSafe3List)
      points <- runRIO env $ Pnt.toPoints safeVertexs
      assertEqual "create points from vectors safelist length == 3"
       [1,2,3,4]
       (map (Env.evalPointId) $ L3.evalSafeList3 points)
 ...
 let
  unique6VertexToPoints = TestCase
   (do
      env <- EnvLdr.loadTestEnvironment
      safeVertexs  <- HexR.runEitherIO "safe vertex" $ ((L3.toSafeList3 [Geo.newVertex  1 1 1, Geo.newVertex  2 2 2,  Geo.newVertex  3 3 3, 
                                                                        Geo.newVertex  11 1 1, Geo.newVertex 12 2 2, Geo.newVertex  13 3 3
                                                                       ]):: Either Hex.HasMeshException L3.VertexSafe3List)
      points <- runRIO env $ Pnt.toPoints safeVertexs
 ...
 let
  unique12VertexToPoints = TestCase
   (do
      env <- EnvLdr.loadTestEnvironment
      safeVertexs  <- HexR.runEitherIO "safe vertex" $ ((L3.toSafeList3 [Geo.newVertex  1 1 1, Geo.newVertex  2 2 2, Geo.newVertex  3 3 3, Geo.newVertex 4 4 4,
 ...
 let
  getSafePointIdListFor3Vectors = TestCase
   (do
      ...
      initialSafeList <- HexR.runEitherIO "initialSafeList" (L3.toSafeList3 [xPointId, yPointId, zPointId]) 
      let
      ...
      eitherPoints <- runRIO env $ Pnt.toPoints' initialVertex initialTailVertexs (L3.reverseSafeList3 initialSafeList )
      assertEqual
       ...
       (case eitherPoints of
          Right rightPoints -> Right $ L3.evalSafeList3 rightPoints
