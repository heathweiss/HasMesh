...
{-
before removing axis and bringing in Polar Cood's
{-# LANGUAGE DeriveDataTypeable #-}
{-# LANGUAGE OverloadedStrings #-}
{-# LANGUAGE NoImplicitPrelude #-}
{-# LANGUAGE PatternSynonyms #-}
...
or import with 'Geometry.Geometry'
-}
module Geometry.Vertex(Vertex(), XAxis(..), YAxis(..), ZAxis(..), pattern Vertex',  newVertex) where
...
To create a Gmsh Id for a Vertex, in the form of a 'Gmsh.ID' 'Gmsh.PointInt', use 'Gmsh.Point.toPoints'.
-}
data Vertex =  Vertex { v_xAxis :: XAxis, v_yAxis :: YAxis, v_zAxis :: ZAxis }
                                                                               
              deriving (Show, Typeable, Data, Eq)

newtype XAxis = XAxis {_xAxis :: Double} deriving (Show, Eq, Typeable, Data)
newtype YAxis = YAxis {_yAxis :: Double} deriving (Show, Eq, Typeable, Data)
newtype ZAxis = ZAxis {_zAxis :: Double} deriving (Show, Eq, Typeable, Data)
  ...
  let
    ...
    round_  val =
      ...
      in
        ...
        fromIntegral(round(val*t))/t
  in
    Vertex (XAxis $ round_ x) (YAxis $ round_ y) (ZAxis $ round_ z)
...
instance H.Hashable Vertex where
    -- | Perhaps this should be hidden, and always just use 'hash'
    hashWithSalt s (Vertex (XAxis x) (YAxis y) (ZAxis z)) =
        s `H.hashWithSalt`
        x `H.hashWithSalt`
        y `H.hashWithSalt` z
    hash vertex =
