...
-}
module Gmsh.IDFx(toPoint, toPoints) where
...
toPoint :: (Enviro.HasPointIdSupply env, Enviro.HasPointIdMap env) => Geo.Vertex -> RIO env ID.PointId
toPoint vertex = do
...
toPoints :: (Enviro.HasPointIdSupply env, Enviro.HasPointIdMap env) => [Geo.Vertex] -> RIO env [ID.PointId]
toPoints [] = return []
toPoints vertexs = do
  let
    toPoints' :: (Enviro.HasPointIdSupply env, Enviro.HasPointIdMap env) => [Geo.Vertex] -> [ID.PointId] -> RIO env [ID.PointId]
